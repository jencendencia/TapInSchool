...
export class SmsQueueWorker {
  ...
  async tick(): Promise<void> {
    ...
    try {
      ...
      // Leader election: only the machine holding the queue lock may dispatch
      // PENDING rows, so two machines can't both send the same SMS. Timeout 0
      // = skip this tick when a peer is already dispatching (it re-runs in
      // 1 s anyway).
